...
function decayAndCluster(db, sessionId) {
  ...
  // Group this session's files by project for cluster detection
  const grouped = getSessionFiles(db, sessionId);
  for (const [project, files] of Object.entries(grouped)) {
    if (files.length >= 2) {
      // Generate all pairs touched together this session
...
module.exports = { bumpFile, decayAndCluster, getSessionFiles, WEIGHTS, DECAY_FACTOR };
